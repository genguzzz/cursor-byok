...
function renderList() {
  ...
  elements.requestList.innerHTML = exchanges
    .map((item) => {
      ...
      return `<tr class="${selected.trim()} server-row-${escapeHTML(server)}" data-id="${escapeHTML(item.id)}">
        <td><span class="row-state ${escapeHTML(item.state)}"></span></td>
        <td><code>${escapeHTML(item.id)}</code></td>
        <td><span class="server-badge ${escapeHTML(server)}">${escapeHTML(serverLabel(server))}</span></td>
...
function renderDetail() {
  ...
  const server = resolveServer(item);
  elements.selectionSummary.innerHTML = `<span class="method-badge">${escapeHTML(item.method)}</span><span class="server-badge ${escapeHTML(server)}">${escapeHTML(serverLabel(server))}</span><span class="status-badge ${statusClass}">${escapeHTML(item.status || formatState(item.state))}</span><code>${escapeHTML(item.url)}</code>`;
...
elements.serverFilter.addEventListener("click", (event) => {
  const button = event.target.closest("button[data-value]");
  if (!button) return;
  state.server = button.dataset.value;
  for (const item of elements.serverFilter.querySelectorAll("button")) {
    item.classList.toggle("active", item === button);
  }
  renderList();
});
