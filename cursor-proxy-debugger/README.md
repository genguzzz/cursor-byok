...
- 其它常见 `aiserver.v1.*` unary（Dashboard / Analytics / Repository / BackgroundComposer / AiService.ServerTime 等）同样走本调试器 MITM，按 `application/proto` 做类型化解码（不是「没进 server」）。
- 若响应是 `text/plain` / `application/json`（含 404 错误页）或 proto 解失败但正文是 UTF-8，会回退写入 `decodedJson` 并合成 1 帧；WebUI 的 frames/body 页也会从 rawHex 回退展示，避免「有字节却空白」。
